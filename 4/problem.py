...
class KHandShakeState:
    ...
    def read_message(self, message, payload_buffer):
        ...
        print("res", self.dh.dh(self.s, self.re))

        # Pattern token: ss
        self.symmetricstate.mix_key(self.dh.dh(self.s, self.rs))
        print("rss", self.dh.dh(self.s, self.rs))
        print("Server cipher key:", self.symmetricstate._ck)
        print("Server hash state:", self.symmetricstate._h)
        # Decrypt payload
...
if __name__ == "__main__":
    ...
    # 6. 自动验证是否成功写入
    actual_data = server.get_user_storage()
    print("\n--- Auto Validation ---")
    if actual_data == target_data:
        print("✓ Storage update successful! You win! 🎉")
        modify_storage_score = 100
    else:
        print("✗ Storage update failed. Data mismatch.")
        print(f"Expected: {target_data}")
        print(f"Actual:   {actual_data}")
        modify_storage_score = 0
